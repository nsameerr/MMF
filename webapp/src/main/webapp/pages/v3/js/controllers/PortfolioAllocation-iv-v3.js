	...
	$scope.onChangeRiskType = function(){
		...
		if (rs != 6){
			$.post(
	        		...
	        		function(rdata) {
	        			//processChartData(rdata); - old code for draw fp chart
		...
		if ($scope.selectedPortfolio == null){
			...
			$.post(
					...
					_gc_url_pa_portfolioChange,
					{
						...
						$scope.$apply();
						//code to plot pie and draw asset class level details
...
//					console.log(rdata);
					try{
						...
						initFinancialPlannerData(rdata,'inv_high_chart');
						//draw rp and 
						drawRiskProfileChart($scope.rpScore,'gauge_section','edit_inputgraphtxt');
						drawRiskProfileSliders($scope.userFpData.savingsRate,$scope.userFpData.fpmasterassumption.fpmiscassumptions.amountInvestedToMmf,$scope.userFpData.fpmasterassumption.fpmiscassumptions.retirementAge,false);
						//advance setting
