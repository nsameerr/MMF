app.controller('AdvisoryServiceContract', ['$scope', function($scope) {
	...
	$scope.reviewConversationDisplay =[];
	$scope.init = function(){
...
//                  console.log($scope.hideReview);
	...
	$scope.setActionUsingStatus = function(){
        ...
        if( $scope.mappingStatusTag == 4 || $scope.mappingStatusTag == 10){
            ...
            var test = angular.copy($scope.contractData.mgmtFeeTypeVariable);
            if(test === 'fixed'){
                id=2;
            }
            if(test === 'variable'){
...
//				console.log(rdata);
	...
	getMgmtFeeVariableAmountList = function (){
		$.ajax({
			...
			success: function(rdata){
				...
				$scope.mgmtFeeVariableAmount = $scope.mgmtFeeVariableAmountList["1"];
				$scope.$apply();
			},
			error: function(xhr, textStatus, error){
				console.log(xhr.statusText);
    ...
    $scope.callToAction = function(buttonId){
        ...
        var text=$('#button-action-'+buttonId).val();
...
//    				console.log(JSON.stringify($scope.contractData));
            		...
            		$.ajax({
    					...
    					success: function(res){
    						  try{
        						  ...
        						  if(res != null && res.redirectUrl != null && res.redirectUrl != ""){
        							  //Redirect to Another Page
        							  window.location.href = _gc_url_baseUrl+res.redirectUrl;
								  }else if(res != null && res.msg != null){
									  // console.log(res.msg);
                                      hideLoading();
                                      showToast(res.msg);
        	...
        	{
    			window.location.href=_gc_url_baseUrl+"/faces/pages/v3/portfolioAllocation.jsp?faces-redirect=true";
        	}
    	}    	
    }
    ...
    $scope.prepareReviewDisplay =  function(){
        $scope.contractData.previousReviews;
        var tempConArray =  $scope.contractData.previousReviews.split('###');
        $scope.reviewConversationDisplay = [];
        for(var i=1;i<tempConArray.length;i++){
            var con = tempConArray[i];
            var obj ;
            if(con.startsWith("INV::")){
                //inv says
                con = con.replace('INV::','');
                var conSplit = con.split(' : ');
                obj =  {isAdv : false, name : conSplit[0], msg : conSplit[1]};
            } else if(con.startsWith("ADV::")){
                //adv says
                con = con.replace('ADV::','');
                var conSplit = con.split(' : ');
                obj =  {isAdv : true, name : conSplit[0], msg : conSplit[1]};
            }
            $scope.reviewConversationDisplay.push(obj);
        }
    }
}]);