			...
			/*if(location.search.length == 0){*/
				...
				$.ajax({
					...
					contentType:"application/json; charset=utf-8",
					  ...
					  $scope.userFpData = data; //.lifeGoals					  
					  $scope.lifeGoalsJson = data.lifeGoals;
					  $scope.$apply();					 		
					  // $scope.processChartData(data);
					  initFinancialPlannerData(data,'inv_portfolio_high_chart');	//ChartId
			...
			if(location.search.length == 0){
				...
				if(viewOnly){
					...
					$.ajax(settings).done(function (rData) {
						...
						// Disable for Testing Only : To Avoid OutOfMemory on Frequent Refresh
						//Set Investor Pic
						 if($scope.investor.profile.masterCustomerQualificationTb != null  && $scope.investor.profile.masterCustomerQualificationTb.invPicPath != null && $scope.investor.profile.masterCustomerQualificationTb.invPicPath != ""){
						 	$scope.investor.pic = !$scope.investor.profile.masterCustomerQualificationTb.invPicPath.match('http') ? '../../images?src='+$scope.investor.profile.masterCustomerQualificationTb.invPicPath :  $scope.investor.profile.masterCustomerQualificationTb.invPicPath;							 
						 	toDataUrl($scope.investor.pic, function(base64Img) {
						 		$scope.investor.pic = base64Img;  
						 		$scope.$apply();
							...
							//Set default pic
							 $scope.investor.pic = "images/default-user.png";
						 }	
...
//		console.log("ActionText : "+invId);
		...
		var settings = {
		  "async": true,
          "crossDomain": true,
          "url": _gc_url_investorProfileInfo,//"/faces/investorProfile",
          "method": "GET",
          "headers": {
            "cache-control": "no-cache",
...
//					$('[id^=button-]').hide();
					...
					$("#div-"+invId).append(res.msg).show();
					//css({"text-align":"center","font-weight": "bold","font-size":"18px"})
					hideLoading();
				}				
			}catch(e){
				console.log('Error');
			...
			if(location.search.length != 0){
				...
				if(viewOnly && $scope.filterTag == false){					
					...
					$.ajax({
						type: "POST",
						url: _gc_url_linkedIn_basicProfile,
						data: "id="+regId+"&userType=INVESTOR",
						dataType: 'json',
						contentType:"application/json; charset=utf-8",
						  success: function(rData){
				                try{
