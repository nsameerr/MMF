 ...
 * @author 07958
 */
@Named("initUserRegistrationBeanV2")
@Scope("prototype")
//@Scope("application")
public class InitUserRegistrationBeanV2 implements Serializable{
    ...
    private String url = PropertiesLoader.getPropertiesValue(LINKED_IN_URL);
...
//        }
    ...
    public void doActionSignUp() {
        status = false;
        LOGGER.info("sumeet doactionsignup:"+ email);
        if (agreeTermsCondition && !email.isEmpty()) {
            ...
            if (userExists) {
                ...
                if (userExists && userRegStatusVO.getRegStatus() == EnumStatus.NEW_APPLICANT.getValue()) {
                	 LOGGER.info(EMAIL_ID_ACTIVATION_IN_PROGRESS);
...
//                            new FacesMessage(FacesMessage.SEVERITY_ERROR, EMAIL_ID_ALREADY_REGISTERED, EMAIL_ID_ALREADY_REGISTERED));
                	 LOGGER.info(EMAIL_ID_ALREADY_REGISTERED);
                }
            } else {
...
//                            new FacesMessage(FacesMessage.SEVERITY_ERROR, "Please accept Terms & Conditions and Privacy Policy", "Please accept Terms & Conditions and Privacy Policy"));
            LOGGER.info("Please accept Terms & Conditions and Privacy Policy");
        }
    ...
    public String signUp() {
    	LOGGER.info("sumeet signup:"+ status);
    	LOGGER.info("sumeet is advisor " + isAdvisor());
    	LOGGER.info("sumeet emial " + getEmail());
...
//            FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put(STORED_VALUES, storedValues);
//            redirectTo = "/pages/init_reg_verify.xhtml?faces-redirect=true";
        	  if(isAdvisor()) {
        		  //advisor
        		  redirectTo = "/faces/pages/init_reg_verify.xhtml?faces-redirect=true";
        	  } else {
        		  // investor
        		  redirectTo = PropertiesLoader.getPropertiesValue(V2_AFTER_SIGN_UP_SUCCESS);  
        	  }
