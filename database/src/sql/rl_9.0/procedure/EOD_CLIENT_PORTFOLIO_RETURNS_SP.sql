...
BEGIN
	...
	SET p_CloseValue = IFNULL((SELECT ROUND(AVG(`close`),2)  FROM `benchmark_performance_tb` WHERE `benchmark` = 
        ...
        /* This IF NOT EXIST Check is done for testing in pre-production and UAT environment.
           On real scenario only the ELSE portion is required.
           Bcoz on real scenario, client balance will be available for each day.
           But in the case of test,it wont we available ,so we take last available balance.
        */
	...
	WHERE `customer_portfolio_id` = P_CUSTOMER_PORTFOLIO_ID AND DATE_FORMAT(`tradedate`,'%Y-%m-%d') = DATE_FORMAT(P_EOD_DATE,'%Y-%m-%d')) THEN
            /* Getting last available client balance */
            SELECT `ledgerbalanec` INTO p_prevBalance FROM `customer_daily_cash_balance_tb`
            WHERE `customer_portfolio_id` = P_CUSTOMER_PORTFOLIO_ID AND DATE_FORMAT(`tradedate`,'%Y-%m-%d') = 
            DATE_FORMAT((SELECT MAX(`tradedate`) FROM `customer_daily_cash_balance_tb` WHERE `customer_portfolio_id` = P_CUSTOMER_PORTFOLIO_ID ),'%Y-%m-%d');
	...
	INSERT INTO `recomended_customer_portfolio_performance_tb`
		...
		SUM(IFNULL(T3.`required_units`,0) * IFNULL(T3.`current_price`,0)) AS market_value_after_cash_flow,
