...
BEGIN
	...
	SET v_Tax = 0.14;
	 ...
	 /* Quarter start from April to March*/
	...
	/*Checking fee type variable/fixed*/
	IF p_FeeVariableType THEN
		SET v_feeVariable = p_FeeVariable/365;
		/* Calculating for Cash flow fee*/
		IF v_CashFlow > 0 THEN
			SET v_cashFlowFee = (v_CashFlow*v_daysLeftToQtr)*v_feeVariable;
		END IF;
		/* Calculating for quarter fee*/
		IF p_Quater THEN
	...
	/*Calculating Fixed Management fee
	  Date is subtracted by one to match with contract date as EOD run on next day
	*/
