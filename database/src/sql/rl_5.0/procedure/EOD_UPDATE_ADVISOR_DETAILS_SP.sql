...
BEGIN
	...
	UPDATE advisor_details_tb c SET c.total_portfolios_managed = 
			(SELECT COUNT(*) FROM customer_portfolio_tb a WHERE a.status=TRUE AND a.advisor_id = p_AdvisorId)
		WHERE c.advisor_id = p_AdvisorId;
		
	SELECT MAX(b.portfolio_90_day_returns),MAX(b.portfolio_1_year_returns), c.outperformance,
			c.outperformance_count_completed, c.outperformance_count_inprogress, c.outperformance_count, c.total_portfolios_managed
		INTO v_MaxPF90DaysReturns, v_MaxPF1YearReturns, v_OutPerformance,
			v_OutPerformanceCountCompleted, v_OutPerformanceCountInProgress, v_OutPerformanceCount, v_TotalPortfoliosManaged
		FROM portfolio_tb b, advisor_details_tb c
		WHERE b.advisor_id = c.advisor_id AND b.status = TRUE AND c.advisor_id = p_AdvisorId;
	
	SELECT v_MaxPF90DaysReturns,v_MaxPF1YearReturns;
	...
	CREATE TEMPORARY TABLE temp_outperform_portfolios_in_style
		SELECT COUNT(*) portfolios_outperformed, a.`portfolio_style` FROM portfolio_tb a, customer_portfolio_tb b
			WHERE a.`portfolio_id` = b.portfolio_id AND b.outperformance = 1 AND  b.advisor_id = p_AdvisorId
			GROUP BY a.`portfolio_style`;
	DROP TEMPORARY TABLE IF EXISTS temp_portfolios_in_style;
	CREATE TEMPORARY TABLE temp_portfolios_in_style
		SELECT COUNT(*) portfolios, a.`portfolio_style` FROM portfolio_tb a, customer_portfolio_tb b
	...
	CREATE TEMPORARY TABLE temp_outperform_rate
		SELECT a.portfolio_style,(a.portfolios_outperformed/b.portfolios) outperformance_rate
			FROM temp_outperform_portfolios_in_style a, temp_portfolios_in_style b
			WHERE a.portfolio_style = b.portfolio_style;
	SELECT d.portfolio_style INTO v_DominantStyle FROM(
	...
	/* Get Max - 90 days / 1 year returns and update */
	/*SELECT MAX(a.`portfolio_90_day_returns`), MAX(a.`portfolio_1_year_returns`) INTO v_Max90DayReturns, v_Max1YearReturns
		FROM customer_portfolio_tb a WHERE a.`advisor_id` = p_AdvisorId;
	UPDATE advisor_details_tb SET `max_returns_90_days` = v_Max90DayReturns, `max_returns_1_year` = v_Max1YearReturns
		WHERE `advisor_id` = p_AdvisorId;*/
    END$$
