...
BEGIN
	...
	**/
	/* ** Calculation of Master Portfolio Returns  Calculation ** */
	...
	INSERT INTO `portfolio_performance_tb`
		...
		IFNULL(T4.`market_value`,SUM(T3.`exe_units` * T3.`current_price`)) AS market_value_after_cash_flow,
		...
		LEFT JOIN `portfolio_performance_tb` T4 
			ON (T4.`portfolio_id` = T1.`portfolio_id` AND T4.`datetime` = DATE_SUB(DATE_FORMAT(P_EOD_DATE,'%Y-%m-%d'),INTERVAL 1 DAY))
		GROUP BY T1.`portfolio_id`;
	
	/* ** Calculation of Master Portfolio Assets Class Returns Calculation ** */
	...
	/* ** Calculation of Master Portfolio Securites Returns Calculation ** */
	...
	INSERT INTO `portfolio_securities_performance_tb`
		...
		LEFT JOIN `portfolio_securities_performance_tb` T4 
			...
			AND T4.`datetime` = DATE_SUB(DATE_FORMAT(P_EOD_DATE,'%Y-%m-%d'),INTERVAL 1 DAY))
