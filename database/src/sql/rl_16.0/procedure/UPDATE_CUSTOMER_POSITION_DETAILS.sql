...
BEGIN
	...
	UPDATE customer_portfolio_securities_tb ps
	INNER JOIN `customer_portfolio_tb` cp ON (ps.`customer_portfolio_id` = cp.customer_portfolio_id)
	INNER JOIN `customer_locked_securities_tb` cl ON(cp.`customer_id`= cl.`customer_id`)
	SET ps.exe_units = ps.exe_units + cl.`securitiy_units`,
	ps.average_rate = cl.`security_price`,ps.lastBoughtPrice = cl.`security_price`
	WHERE ps.`venueCode` = 'NSEMF' AND ps.`security_code` = cl.`security_code` AND ps.`venueScriptCode` = cl.`venue_script_code` AND cl.`processed`=0;
	
	UPDATE `customer_locked_securities_tb` SET `processed`=1;
