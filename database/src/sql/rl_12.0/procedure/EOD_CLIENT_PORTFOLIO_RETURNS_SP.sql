...
BEGIN
	...
	SELECT oms_login_id INTO p_omsId FROM `customer_portfolio_tb` WHERE `customer_portfolio_id` = P_CUSTOMER_PORTFOLIO_ID;
	...
	+T1.`cash_amount`,SUM(T3.`current_price` * T3.`exe_units`)+T1.`cash_amount`)
         ELSE  SUM(T3.`current_price` * (T3.`exe_units`+ IFNULL(T1.`temp_blocked_count`,0)+IFNULL(T1.`blocked_count`,0)))+T1.`cash_amount` END 
