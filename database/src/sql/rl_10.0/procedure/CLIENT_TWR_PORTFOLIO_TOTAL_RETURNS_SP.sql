...
DROP PROCEDURE IF EXISTS `client_twr_portfolio_total_returns_sp`$$

CREATE DEFINER=`mmfuser`@`%` PROCEDURE `client_twr_portfolio_total_returns_sp`(  	
...
BEGIN
		...
		DECLARE cursor_client_pf_return CURSOR FOR 
			SELECT (1 + IFNULL(T1.sub_period_return,0)) FROM `customer_portfolio_performance_tb` T1
			WHERE T1.`customer_portfolio_id` = P_CUST_PORTFOLIO_ID 
		...
		DECLARE cursor_client_pf_return2 CURSOR FOR 
			SELECT (1 + IFNULL(T1.sub_period_return,0)) FROM `customer_portfolio_performance_tb` T1
			WHERE T1.`customer_portfolio_id` = P_CUST_PORTFOLIO_ID ;
		...
		IF(P_NO_OF_BACK_DAYS > 0) THEN
			...
			OPEN cursor_client_pf_return;
			
			label_exit_loop: LOOP
			
				FETCH cursor_client_pf_return INTO v_sub_return;
			...
			CLOSE cursor_client_pf_return;
			
			SELECT  v_TWR;
		
		ELSE
		
			OPEN cursor_client_pf_return2;
			
			label_exit_loop: LOOP
			
				FETCH cursor_client_pf_return2 INTO v_sub_return;
				IF v_cursor_finished  THEN
			...
			CLOSE cursor_client_pf_return2;
