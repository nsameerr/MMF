...
DROP PROCEDURE IF EXISTS `recommended_twr_total_portfolio_returns_sp`$$

CREATE DEFINER=`mmfuser`@`%` PROCEDURE `recommended_twr_total_portfolio_returns_sp`(		
...
BEGIN
		...
		DECLARE cursor_recommended_portfolio_return CURSOR FOR 
			...
			SELECT (1 + IFNULL(T1.sub_period_return,0)) FROM `portfolio_performance_tb` T1
			WHERE T1.`portfolio_id` = P_PORTFOLIO_ID 
