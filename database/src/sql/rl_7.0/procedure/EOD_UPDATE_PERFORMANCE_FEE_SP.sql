...
BEGIN
	...
	IF v_PerfAboveThreshold > 0.0 THEN
		...
		SELECT a.opening_portfolio_value INTO v_CustomerPortfolioStartValue FROM customer_portfolio_log_tb a
			WHERE a.customer_portfolio_id = v_CustomerPortfolioId AND DATE_FORMAT(DATE(a.log_date),'%Y-%m-%d') = DATE_FORMAT(DATE(v_StartDate),'%Y-%m-%d');
		
		SELECT a.closing_portfolio_value INTO v_CustomerPortfolioCloseValue FROM customer_portfolio_log_tb a
			WHERE a.customer_portfolio_id = v_CustomerPortfolioId AND DATE_FORMAT(DATE(a.log_date),'%Y-%m-%d') = DATE_FORMAT(DATE(p_CurrentDatetime),'%Y-%m-%d');
	...
	UPDATE customer_advisor_mapping_tb a SET a.perf_fee_last_calculated = p_CurrentDatetime, a.perf_fee_amt = v_PerformanceFee
		...
		/*Customer performance fee is adding to the table*/
   
   
        INSERT  INTO customer_performance_fee_tb (relation_id,customer_id,advisor_id,fee_calculate_date,perf_fee,perf_fee_freq,ecs_pay_date,ecs_paid,reg_id)
        SELECT  p_RelationId,c.customer_id,b.advisor_id,p_CurrentDatetime,v_PerformanceFee,a.perf_fee_freq, p_EcsDueDate AS ecsPayDate,FALSE,c.registration_id
	...
	SELECT v_PerformanceFee performanceFee, p_RelationId relatioId,
			b.advisor_id advisorId,CONCAT(b.first_name,' ',b.`middle_name`,' ',b.`last_name`) advisorFirstName, b.email advisorEmail,
