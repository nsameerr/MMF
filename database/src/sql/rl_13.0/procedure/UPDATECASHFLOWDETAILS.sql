...
BEGIN
	...
	IF (DATE_FORMAT(trDate ,'%Y-%m-%d')= DATE_SUB(DATE_FORMAT(NOW(),'%Y-%m-%d'),INTERVAL 1 DAY)) THEN	
		...
		IF ((SELECT COUNT(*) FROM `customer_portfolio_tb` WHERE `oms_login_id`= OMS_ID AND `portfolio_status`='ACTIVE') > 0) THEN
		
			SET p_investorID = (SELECT customer_id FROM customer_portfolio_tb WHERE oms_login_id= OMS_ID AND `portfolio_status`='ACTIVE');
			SET p_relationID = (SELECT `relation_id` FROM customer_portfolio_tb WHERE oms_login_id= OMS_ID AND `portfolio_status`='ACTIVE');
			SET p_allocateFund = (SELECT `allocatedFund` FROM customer_portfolio_tb WHERE oms_login_id= OMS_ID AND `portfolio_status`='ACTIVE'); 
			...
			WHERE oms_login_id= OMS_ID AND `portfolio_status`='ACTIVE';
