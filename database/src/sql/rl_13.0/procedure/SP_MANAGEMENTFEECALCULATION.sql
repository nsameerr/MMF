...
BEGIN
	...
	SELECT `cashFlow` INTO v_CashFlow FROM `customer_portfolio_tb` WHERE `relation_id` = p_RelationId AND `portfolio_status`='ACTIVE';
	
	SELECT `actual_contract_end_date` INTO v_contractEndDate FROM `customer_advisor_mapping_tb` WHERE `relation_id`= p_RelationId;
	...
	IF p_FeeVariableType THEN
		...
		ELSEIF (DATE_FORMAT(p_ContractDate,'%Y-%m-%d') = DATE_SUB(DATE_FORMAT(NOW(),'%Y-%m-%d'),INTERVAL 1 DAY)) THEN
			SET v_qtrBillAmt = (p_TotalAllocatedFund*v_daysLeftToQtr)*v_feeVariable;
		ELSEIF (DATE_FORMAT(v_contractEndDate,'%Y-%m-%d') = DATE_SUB(DATE_FORMAT(NOW(),'%Y-%m-%d'),INTERVAL 1 DAY)) THEN
			SET v_qtrBillAmt = (p_TotalAllocatedFund*v_daysLeftToQtr)*v_feeVariable;	
		ELSE
