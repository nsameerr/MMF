...
BEGIN
	...
	DECLARE p_omsId VARCHAR(10);
	DECLARE p_count INT DEFAULT 0; 
	
	SET P_EOD_DATE = IF(P_EOD_DATE = '',NULL,P_EOD_DATE);
	
	SELECT oms_login_id INTO p_omsId FROM `customer_portfolio_tb` WHERE `customer_portfolio_id` = P_CUSTOMER_PORTFOLIO_ID AND portfolio_status ='ACTIVE';
	SELECT COUNT(*) INTO p_count FROM `customer_transaction_execution_details_tb` WHERE `oms_userid` = p_omsId;
	
	SET p_CloseValue = IFNULL((SELECT ROUND(AVG(`close`),2)  FROM `benchmark_performance_tb` WHERE `benchmark` = 
                                    (SELECT IF(`venue_name` = "NSE",'1','2') FROM customer_portfolio_tb WHERE customer_portfolio_id = P_CUSTOMER_PORTFOLIO_ID) 
                                    AND DATE_FORMAT(`datetime`,'%Y-%m-%d')= DATE_FORMAT(P_EOD_DATE,'%Y-%m-%d')),
                                        (SELECT ROUND(AVG(bp.`close`),2)  FROM `benchmark_performance_tb` bp WHERE bp.`benchmark` = 
                                        (SELECT IF(`venue_name` = "NSE",'1','2') FROM customer_portfolio_tb WHERE customer_portfolio_id = P_CUSTOMER_PORTFOLIO_ID) 
                                        AND DATE_FORMAT(bp.`datetime`,'%Y-%m-%d')= DATE_FORMAT((SELECT MAX(`datetime`) FROM `benchmark_performance_tb`),'%Y-%m-%d')));
        
	IF NOT EXISTS(SELECT `ledgerbalanec` FROM `customer_daily_cash_balance_tb`
	WHERE `customer_portfolio_id` = P_CUSTOMER_PORTFOLIO_ID AND DATE_FORMAT(`tradedate`,'%Y-%m-%d') = DATE_FORMAT(P_EOD_DATE,'%Y-%m-%d')) THEN
	
            SELECT `ledgerbalanec` INTO p_prevBalance FROM `customer_daily_cash_balance_tb`
            WHERE `customer_portfolio_id` = P_CUSTOMER_PORTFOLIO_ID AND DATE_FORMAT(`lastupdatedon`,'%Y-%m-%d') = 
            DATE_FORMAT((SELECT MAX(`lastupdatedon`) FROM `customer_daily_cash_balance_tb` WHERE `customer_portfolio_id` = P_CUSTOMER_PORTFOLIO_ID ),'%Y-%m-%d');
	...
	ELSE
            ...
            DATE_FORMAT(`tradedate`,'%Y-%m-%d') = DATE_FORMAT(P_EOD_DATE,'%Y-%m-%d');
	...
	SET ORDER_EXECUTED = IF(IFNULL(p_count,0) > 0,1,0);
	...
	CASE WHEN ORDER_EXECUTED = 1 THEN IFNULL(ROUND(SUM(ROUND(T3.`current_price`,2) * IF(T4.`buy_sell`= 'B',T4.`security_units`,-T4.`security_units`)),2),0)
         ELSE 0.00 END AS closing_value,	
         SUM(T3.`current_price` * T3.`yesterDayUnitCount`)+p_prevBalance+IFNULL(T1.`blocked_cash`,0)+IFNULL(T1.`temp_blocked_cash`,0) AS market_value,
	 T1.`cashFlow` AS cash_flow,
	CASE WHEN ORDER_EXECUTED = 1 THEN IFNULL(IFNULL(SUM(T3.`current_price` * T3.`yesterDayUnitCount`),0) + SUM(T4.`security_price` * IF(T4.`buy_sell`= 'B',T4.`security_units`,-T4.`security_units`))
	+T1.`cash_amount`+ IFNULL(T1.`blocked_cash`,0)+IFNULL(T1.`temp_blocked_cash`,0),SUM(T3.`current_price` * T3.`exe_units`)+T1.`cash_amount`)
         ELSE  SUM(T3.`current_price` * (T3.`exe_units`+ IFNULL(T3.`blocked_count`,0)))+T1.`cash_amount`+ IFNULL(T1.`blocked_cash`,0)+IFNULL(T1.`temp_blocked_cash`,0) END 
         AS market_value_after_cashflow,
        IFNULL((IFNULL(SUM(T3.`current_price` * T3.`yesterDayUnitCount`)+p_prevBalance+IFNULL(T1.`blocked_cash`,0)+IFNULL(T1.`temp_blocked_cash`,0),0) - IFNULL(T5.`market_value_after_cash_flow`,0) ) / IFNULL(T5.`market_value_after_cash_flow`,0),0.0) 
	...
	LEFT JOIN `customer_portfolio_performance_tb` T5 
        ...
        WHERE T1.`customer_portfolio_id` = P_CUSTOMER_PORTFOLIO_ID  AND T1.`portfolio_status` = 'ACTIVE' GROUP BY T1.`customer_portfolio_id`;	
	
	
	
	INSERT INTO `recomended_customer_portfolio_performance_tb`
		(
		`customer_portfolio_id`,`datetime`,`closing_value`,`market_value`,`cash_flow`,`market_value_after_cash_flow`,`sub_period_return`
		)
		SELECT T1.`customer_portfolio_id`,P_EOD_DATE,
		...
		T1.`recomended_cash_bal` AS cash_flow,
		SUM(IFNULL(T3.`required_units`,0) * IFNULL(T3.`current_price`,0)) + T1.`recomended_cash_bal` AS market_value_after_cash_flow,
		IFNULL((SUM(IFNULL(T3.`yesterDayUnitCount`,0) * IFNULL(T3.`current_price`,0)) - T4.`market_value_after_cash_flow`) / T4.`market_value_after_cash_flow`,0) AS sub_period_return
		FROM `customer_portfolio_tb` T1
		INNER JOIN `customer_portfolio_details_tb` T2 ON ( T2.`customer_portfolio_id` = T1.`customer_portfolio_id`)
		...
		LEFT JOIN `recomended_customer_portfolio_performance_tb` T4 
			ON (T4.`customer_portfolio_id` = T1.`customer_portfolio_id` 
			AND DATE_FORMAT(T4.`datetime`,'%Y-%m-%d') = DATE_SUB(DATE_FORMAT(P_EOD_DATE,'%Y-%m-%d'),INTERVAL 1 DAY))
		WHERE T3.`status` =TRUE AND T1.`customer_portfolio_id` = P_CUSTOMER_PORTFOLIO_ID AND T1.`portfolio_status` = 'ACTIVE'  GROUP BY T1.`customer_portfolio_id`; 
	...
	WHERE T1.`customer_portfolio_id` = P_CUSTOMER_PORTFOLIO_ID AND T1.`portfolio_status` = 'ACTIVE'
	GROUP BY T1.`customer_id`,T3.`asset_class_id`;
	...
	CASE WHEN ORDER_EXECUTED = 1 THEN IFNULL(SUM(ROUND(T3.`current_price`,2) * IF(T4.`buy_sell`= 'B',T4.`security_units`,-T4.`security_units`)),0)  
        ELSE 0.00 END AS closing_value,
	IFNULL(SUM(IF(T4.`buy_sell`= BUY,T4.`security_units`,-T4.`security_units`)),0) AS existing_units,
	T3.`current_price`,
    	SUM(T3.`current_price` * T3.`yesterDayUnitCount`) AS market_value,
	CASE WHEN ORDER_EXECUTED = 1 THEN IFNULL(SUM(T4.`security_price` * IF(T4.`buy_sell`= 'B',T4.`security_units`,-T4.`security_units`)),0)
        ELSE 0 END AS cash_flow,
	CASE WHEN ORDER_EXECUTED = 1 THEN IFNULL(IFNULL(SUM(T3.`current_price` * T3.`yesterDayUnitCount`),0) + SUM(T4.`security_price` * IF(T4.`buy_sell`= 'B',T4.`security_units`,-T4.`security_units`)),SUM(T3.`current_price` * T3.`exe_units`))
        ELSE SUM(T3.`current_price` * T3.`exe_units`) END AS market_value_after_cashflow,
	...
	UPDATE customer_transaction_execution_details_tb
	SET `processed` = TRUE
	WHERE `oms_userid` = p_omsId;
	-- TRUNCATE TABLE `cash_txn_for_the_day`;		
